 ...
 */
public class DaoGou2 {

    HttpClient httpClient = HttpClients.createDefault();

    public double getGzRepair(String code){
        double result = 0.0;
        ...
        if(text.contains("净值估算平均偏差：") && text.contains("%")){
            result =  new Double(text.replace("净值估算平均偏差：","").replace("%",""));
        }
        return result;
    }
...
//        System.out.println(url);
    ...
    public Double getYearValue(String code) throws Exception {
        Double result = 0.0;
    ...
    public void getInfo() throws Exception {
        List<String> list = TxtUtil.readTxt(new File("100.txt"));
...
//            double oneValue = x[18];
            ...
            //平均估值偏差
            double gzRepair = getGzRepair(code);
            //接近真实的估值百分比
...
//            boolean isBuy = false;
//            //连续三日下跌 并且下跌幅度大于-3
//            if( gzPercent<0&&(twoPercent+onePercent+currentPercent+gzPercent)<-3){
//                isBuy=true;
//            //连续2日下跌 并且下跌幅度大于-4
...
//                    System.out.println(currentValue);
                    ...
                    sb.append(code + "\t" + currentValue + "\t" + gzValue + "\t" + gzPercent + "\t" + map.get(code) + "\t" + status + "\n");
                    sb2.append("33#" + code + "\n");
                } else {
        ...
        TxtUtil.writeTxt("D:\\BaiduNetdiskDownload\\000EBK\\100_buy.txt", sb.toString());
        TxtUtil.writeTxt("D:\\BaiduNetdiskDownload\\000EBK\\100_buy.EBK", sb2.toString());
        TxtUtil.writeTxt("D:\\BaiduNetdiskDownload\\000EBK\\100_sell.txt", sb3.toString());
        TxtUtil.writeTxt("D:\\BaiduNetdiskDownload\\000EBK\\100_sell.EBK", sb4.toString());
        getInfo11111();
...
//    @Test
    public void getInfo11111() throws Exception {
        List<String> valueList = TxtUtil.readTxt(new File("D:\\BaiduNetdiskDownload\\000EBK\\100_buy.EBK"));
        StringBuilder sb3 = new StringBuilder();
        Map<String,Double> map = new HashMap<>();
        for (int i = 0; i < valueList.size(); i++) {
            String code = valueList.get(i).split("#")[1];
            Double value = getYearValue(code);
            if(value==0.0){
                continue;
            }
            sb3.append(code + "\t" + value + "\n");
            map.put(code,value);
        }
        TxtUtil.writeTxt("D:\\BaiduNetdiskDownload\\000EBK\\compare100.txt", MapUtil.sortDescInfo(map));
    }
    ...
    public void getPool2() throws Exception {
        String[] array = new String[]{"z", "1y", "3y", "6y", "jn", "1n"};
        StringBuilder sb = new StringBuilder();
...
//            System.out.println(list.size());
        ...
        for (int i = 0; i < list2.size(); i++) {
            ...
            sb2.append("33#"+list2.get(i).split("\t")[0]+"\n");
        }
        TxtUtil.writeTxt(new File("pool2.txt"), sb.toString());
        TxtUtil.writeTxt(new File("D:\\BaiduNetdiskDownload\\000EBK\\888.EBK"), sb2.toString());
    }

    @Test
    public void getJn100() throws Exception {
        String[] array = new String[]{  "1n"};
        StringBuilder sb = new StringBuilder();
        List<String> list = new ArrayList<>();
        for (int i = 0; i < array.length; i++) {
            String type = array[i];
...
//            sb2.append("33#"+list2.get(i).split("\t")[0]+"\n");
//        }
        TxtUtil.writeTxt(new File("100.txt"), sb.toString());
//        TxtUtil.writeTxt(new File("D:\\BaiduNetdiskDownload\\000EBK\\888.EBK"), sb2.toString());
    }
